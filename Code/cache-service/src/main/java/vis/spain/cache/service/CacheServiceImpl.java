...
public class CacheServiceImpl implements CacheService {
	...
	public void updateCache() throws GeneralSystemFailureException {
		...
		servicisList[0].setCategory("asd");
		...
		mongoTemplate.insert(servicisList, ServiceDataFromRP[].class);
		/*
		 * BulkOperations bulkOperations =
		 * mongoTemplate.bulkOps(BulkMode.UNORDERED, ServiceDataFromRP.class);
		 * 
		 * BulkOperations bulkOperations =
		 * mongoTemplate.bulkOps(BulkMode.UNORDERED, ServiceDataFromRP.class);
		 * List<Pair<Query, Update>> catalogUpsert = new ArrayList<>();
		 * List<String> codes = new ArrayList<>(); for (ServiceDataFromRP
		 * catalog : servicisList) { DBObject dbObject = new BasicDBObject();
		 * mongoTemplate.getConverter().write(catalog, dbObject); Pair<Query,
		 * Update> pair = Pair.<Query,
		 * Update>of(Query.query(Criteria.where("code").is(catalog.getCode())),
		 * Update.fromDBObject(dbObject)); catalogUpsert.add(pair);
		 * codes.add(catalog.getCode()); } bulkOperations.(servicisList);
